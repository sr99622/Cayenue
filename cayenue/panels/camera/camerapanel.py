...
class CameraPanel(QWidget):
    ...
    def getProxyData(self, onvif_data):
        ...
        if not self.getCameraBySerialNumber(onvif_data.serial_number()):
            ...
            if add_camera:
                ...
                logger.debug(f'Discovery completed for Camera: {camera.name()}, Stream URI: {onvif_data.stream_uri()}, xaddrs: {onvif_data.xaddrs()}')
    ...
    def getData(self, onvif_data):
        ...
        if existing := self.getCameraBySerialNumber(onvif_data.serial_number()):
            ...
            camera.setDisplayProfile(camera.getDisplayProfileSetting())
            self.saveCameraList()
            logger.debug(f'Discovery completed for Camera: {camera.name()}, Stream URI: {onvif_data.stream_uri()}, xaddrs: {onvif_data.xaddrs()}, {onvif_data.camera_name()}')
    ...
    def btnHistoryClicked(self):
        try:
            ...
            reader_settings.setValue("settings/audioDriverIndex", self.mw.settingsPanel.general.cmbAudioDriver.currentIndex())
